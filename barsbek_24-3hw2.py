zodiac_sign =input('Укажите день и месяц рождения:')
date , month = int(zodiac_sign[:2]) , int(zodiac_sign[3:])
if (date>=21 and date<=31 and month==3) or( month==4 and date>=1 and date<=20):
   print("Знак зодиака:Овен")
elif (date>=21 and date<=30 and month==4) or( month== 5and date>=1 and date<=21):
...
elif (date>=23 and date<=31 and month==8) or( month==9 and date>=1 and date<=21):
